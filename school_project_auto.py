...
# play_record = open(current_path + "\\" + "play_record_2.txt","a")
...
# 파일이 있으면 실행
if os.path.isfile(os.path.join(current_path, 'play_record_2.csv')):
    # 데이터 가져오기
    df = pd.read_csv(os.path.join(current_path,'play_record_2.csv'))

    #fps만 뽑아 비교
    fps = df['fps']
    fps_list = list(fps.values)
    max_f = max(fps_list)
    ...
    for i in range(1, max_f+1):
        ...
        a = randint(0,len(frame_values)-1)
        # 렌덤으로 뽑은  플레이 저장
        p2_event_list.append(list(frame_values[a]))
...
while running:
    ...
    # player1 AI
    player1_event ={'to_en':False, 'against_en': False, 'jump':False, 'attack': False, 'shield': False}
    movement_p1=randint(0,999)

    if movement_p1%10 <= int(int(2*abs(p1_x_pos -p2_x_pos))/100):
            player1_event['to_en']=True
    elif int(movement_p1/10)%10 >= int(7+int(abs(p1_x_pos -p2_x_pos))/100):
                player1_event['attack']=True
    elif int(movement_p1/10)%10 < int(3+int(abs(p1_x_pos -p2_x_pos))/100):
                player1_event['against_en']=True
    elif int(movement_p1/100) < int(7-int(abs(p1_x_pos -p2_x_pos))/100):
    ...
    for event in player1_event: 
        if move_p1:
                
            if player1_event['to_en']:
                if p2_x_pos -p1_x_pos >= 0:
                    to_x_p1 = speed_p1
                    to_p1 = 0
                elif p2_x_pos -p1_x_pos < 0:
                    to_x_p1 = -speed_p1
                    to_p1 = 1
            if player1_event['against_en']:
                if p2_x_pos -p1_x_pos >= 0:
                    to_x_p1 = -speed_p1
                    to_p1 = 1
                elif p2_x_pos -p1_x_pos < 0:
        ...
        if SPACE_p1:
            if player1_event['attack']:
                if p1_y_pos == screen_height - stage_height - p1_height:
                    ...
                    SHIFT_p1 = False
        if SHIFT_p1:
            if player1_event['shield']:
                if p1_y_pos == screen_height - stage_height - p1_height:
                    to_x_p1 = 0
                    move_p1 = False
                    shield_p1 = True
                    SPACE_p1 =False
            if player1_event['shield'] == False:
                shield_p1 = False
                move_p1 = True
                SPACE_p1 = True
...
if ending_txt != "None":
    if os.path.isfile(os.path.join(current_path,'play_record_2.csv')):
        df_read = pd.read_csv(os.path.join(current_path,'play_record_2.csv'))
        df = df_read.append(df,ignore_index = True)
    df.to_csv(os.path.join(current_path,'play_record_2.csv'),index = False)



if os.path.isfile(os.path.join(current_path,'game_record_1.txt')):
    with open(os.path.join(current_path,'game_record_1.txt'),"rb") as fr:
        data = pickle.load(fr)
    if ending_txt == "Blue Win":
        data['BLUE']+=1
    if ending_txt == "Red Win":
